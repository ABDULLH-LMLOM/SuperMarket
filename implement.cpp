...
int  product::edit_product(string code)
{
    fstream data ;
    data.open("database.txt",ios::in);
    int loop =num_object; string code_in , name_in; int salary_in;
    while(loop--){
        data >> code_in >> name_in >> salary_in;
        codes.push_back(code_in);names.push_back(name_in); salarys.push_back(salary_in);
    }
    data.close();
    int index=-1 ; 
    for(int i=0 ; i< num_object ; i++){
...
void product::delete_product(string code)
{
    fstream data ; 
    data.open("database.txt",ios::in);
    string code_d , name_d ; int salary_d ;
    for(int i=0 ;i < num_object ; i++){
        data>>code_d>>name_d>>salary_d;
        if(code_d != code)
        {
            codes.push_back(code_d);names.push_back(name_d);salarys.push_back(salary_d);  
        }
    }
    data.close();
    data.open("database.txt",ios::out);
